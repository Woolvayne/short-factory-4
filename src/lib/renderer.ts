...
function drawCaption(
  ...
  const fs = Math.round(w * s.captionScale);
  ctx.font = `900 ${fs}px "Arial Black", Arial, Helvetica, sans-serif`;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";

  const maxW = w * 0.85;
  const words = (s.uppercase ? text.toUpperCase() : text).split(/\s+/);
  const lines: string[] = [];
  let cur = "";
  ...
  const stroke = Math.round(fs * s.outlineWidth);
  lines.forEach((ln, i) => {
...
export async function renderLocal(opts: RenderJobOptions): Promise<LocalRenderResult> {
  ...
  try {
    ...
    if (opts.musicFile && s.musicVolume > 0) {
      try {
    ...
    if (musicSrc && musicGain) {
      ...
      if (s.musicFade) {
        musicGain.gain.setValueAtTime(s.musicVolume, Math.max(startAt, endAt - 1.4));
