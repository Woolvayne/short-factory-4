...
export default function App() {
  ...
  const jumpTo = useCallback((section: "factory" | "calendar") => {
    const target = section === "calendar" ? calendarRef.current : factoryRef.current;
  ...
  return (
    <div className="grain relative min-h-dvh bg-coal-950">
      ...
      >
        ...
        {/* ---------------- Social scheduling: dashboard + calendar ---------------- */}
        <div ref={calendarRef} className="mt-10 scroll-mt-24">
          <div className="mb-5 flex items-start gap-2 text-coal-500">
            <ArrowDown className="mt-0.5 size-3.5 shrink-0" />
            <span className="mono-label text-[9px] leading-relaxed">
              06 · SOCIAL DISPATCH — DASHBOARD, KALENDER UND GEPLANTE VERÖFFENTLICHUNGEN
            </span>
          </div>
          <CalendarView
